...
docker image build -f debian.Dockerfile \
    -t travissouth/terraform-aws-cli:1.0.0-debian-${TERRAFORM_VERSION}-${AWS_CLI_VERSION} \
    -t travissouth/terraform-aws-cli:debian-latest \
    .

# Build the Alpine based image:
docker image build -f alpine.Dockerfile \
    -t travissouth/terraform-aws-cli:1.0.0-alpine-${TERRAFORM_VERSION}-${AWS_CLI_VERSION} \
    -t travissouth/terraform-aws-cli:alpine-latest \
    .

docker push travissouth/terraform-aws-cli:debian-latest
docker push travissouth/terraform-aws-cli:1.0.0-debian-${TERRAFORM_VERSION}-${AWS_CLI_VERSION}
docker push travissouth/terraform-aws-cli:alpine-latest
docker push travissouth/terraform-aws-cli:1.0.0-alpine-${TERRAFORM_VERSION}-${AWS_CLI_VERSION}
